...
class P79ViewModel extends BaseViewModel {
  ...
  void P79Next(DoiSongHoModel data) async {
    await _executeDatabase.updateDSH("SET c62_M4 = ? WHERE idho = ? AND thangDT = ? AND namDT = ?",
        [data.c62_M4, data.idho, data.thangDT, data.namDT]);
    if(data.c62_M4 == 3) {
      NavigationServices.instance.navigateToP80(context);
    } else {
      await _executeDatabase.updateDSH("SET c62_M5A = ?, c62_M5B = ?, c62_M5C = ?, "
          "c62_M5D = ?, c62_M5E = ?, c62_M5F = ?, c62_M5G = ?, c62_M5H = ?, "
          "c62_M5I = ?, c62_M5IK = ? WHERE idho = ? AND thangDT = ? AND namDT = ?",
